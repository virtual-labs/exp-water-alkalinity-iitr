// Read stored experiment data (do NOT assign to globals here - they are declared later)
const _storedExperiment = ExperimentStorage.getExperimentData() || {};
const stored_var1 = parseFloat(_storedExperiment.normality_titrate || _storedExperiment.Normality_titrate || 0.016); // Normality of titrate
const stored_var2 = parseFloat(_storedExperiment.volume_titrant || _storedExperiment.volume_titrate || _storedExperiment.volume_titrant2 || 10); // Volume of sample (ml)
...
// Track loaded images for preload
let loadedImages = 0;
...
function setup() {
  ...
  // Initialize globals from stored values read earlier
  normality_titrate = stored_var1;
  volume_titrate = parseFloat(stored_var2);
  old = parseFloat(stored_var3);
...
function start() {
  ...
  // Guard slider2/slider3 disabling: they may not exist on this page
  if (typeof slider2 !== 'undefined' && slider2) {
    try { slider2.attribute('disabled', true); } catch(e){}
  }
  if (typeof slider3 !== 'undefined' && slider3) {
    try { slider3.attribute('disabled', true); } catch(e){}
  }
...
function nextpressed(){
  // Save second titration results into localStorage and redirect to results page
  let exp = ExperimentStorage.getExperimentData() || {};
  // Save fields using both conventions (uppercase keys used previously by PHP sessions)
  exp.Normality_titrate2 = normality_titrate;
  exp.volume_titrant2 = volume_titrant;
  exp.volume_titrate2 = volume_titrate;
  exp.Normality_titrant2 = normality_titrant;

  // Also store lowercase variants to keep earlier code compatible
  exp.normality_titrate2 = normality_titrate;
  exp.volume_titrant2 = volume_titrant;
  exp.volume_titrate2 = volume_titrate;
  exp.normality_titrant2 = normality_titrant;

  localStorage.setItem('experimentData', JSON.stringify(exp));
  console.log('Saved second titration data:', exp);
